...
<head>
  ...
  <title>Waiting for camera...</title>
  <style>
...
<body>
  <main class="stage" id="stage">
    <h1>Waiting for camera image</h1>
    ...
    (function(){
      ...
      function connect(){
        ...
        ws.addEventListener('open', () => {
          setStatus('Connected webcam image');
          try{ ws.send('bix/img:ping'); }catch(e){console.warn(e)}
        });

        ws.addEventListener('message', (ev) => {
          console.log('ws message', ev.data);
          if (ev.data == "ping") return setStatus('Connected'); //console.log("ping received");
          setStatus('loading new image...');
	  ...
	  const targetUrl = 'last?' + "<?php echo date_timestamp_get(date_create()) ?>"//+ encodeURIComponent(payload);
          fetch(targetUrl, { credentials: 'same-origin' })
            ...
            .catch(err => {
              ...
              setStatus('Failed to load image. See console for details.');
            });
        });
